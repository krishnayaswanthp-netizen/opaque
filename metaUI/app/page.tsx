        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="space-y-2 text-center">
            <p
              ...
              `}
            >
              Drop a real user image to inspect its hidden metadata
            </p>
            <p className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
              <FileImage className="w-4 h-4" />
              Supports PNG, JPG, JPEG, TIF, and TIFF only
            </p>
...
function ActionPane({
  ...
  return (
    <div className="glass-panel space-y-4 rounded-xl p-6">
      ...
      <p className="text-sm text-muted-foreground">
        {hasSensitiveMetadata
          ? "Meta-Shield will re-encode the image, verify the output, and keep a clean artifact ready for download."
...
export default function OpaquePage() {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      appendScanLog("Extracting metadata from the uploaded image", "info")
      ...
      const deviceFingerprint = [report.device.make, report.device.model].filter(Boolean).join(" ")
      if (deviceFingerprint) {
        appendScanLog(`Device fingerprint found: ${deviceFingerprint}`, "error")
      }

      if (Object.keys(report.timestamps).length) {
        appendScanLog("Timestamp metadata detected in the uploaded image.", "error")
      }
